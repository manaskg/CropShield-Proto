...
import { TrendingUp, Sprout, Ruler, CloudSun, Loader2, Calendar, Award, CheckCircle, AlertCircle } from 'lucide-react';
...
const SmartFarm = () => {
  ...
  const handleGenerate = async () => {
    ...
    setError(null);
    try {
      const res = await aiApi.getYieldPlan(formData.crop, formData.acres, formData.season, getLanguageName());
      const generatedPlan = res.plan || res.yieldPlan || res;
      if (generatedPlan && (generatedPlan.expectedYield || generatedPlan.timeline)) {
        setPlan(generatedPlan);
      } else {
        throw new Error("No plan data received from AI model");
      }
    } catch (e) {
      console.error("Yield plan error:", e);
      setError(e?.response?.data?.message || e?.message || "Failed to generate yield plan. Please try again.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-stone-50 pt-24 pb-12 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        {error && (
          <div className="max-w-md mx-auto mb-6 bg-red-50 border-l-4 border-red-500 p-4 rounded-r-xl text-red-700 text-sm flex items-center gap-2">
            <AlertCircle size={18} />
            <span>{error}</span>
          </div>
        )}
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 bg-white p-8 rounded-3xl shadow-xl border border-stone-100 h-fit">
            <h3 className="font-bold text-stone-800 mb-6 uppercase tracking-wider text-sm border-b border-stone-100 pb-2">
              {t('yield.config_title')}
            </h3>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-3.5 border border-stone-200 rounded-xl bg-stone-50 focus:ring-2 focus:ring-emerald-500 outline-none font-medium"
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <select
                    ...
                    className="w-full pl-12 pr-4 py-3.5 border border-stone-200 rounded-xl bg-stone-50 focus:ring-2 focus:ring-emerald-500 outline-none appearance-none font-medium"
                  >
                ...
                {plan.timeline && (
                  <div className="bg-white rounded-3xl p-8 shadow-xl border border-stone-100">
                    <h3 className="text-lg font-bold text-stone-900 mb-8 flex items-center gap-2">
                      <Calendar className="text-emerald-600"/> {t('yield.res.timeline')}
                    </h3>
                ...
                {plan.generalTips && (
                  <div className="bg-amber-50 rounded-3xl p-8 border border-amber-100">
                    <h3 className="text-lg font-bold text-amber-900 mb-4 flex items-center gap-2">
                      <Award size={20}/> {t('yield.res.tips')}
                    </h3>
