...
const Profile = ({ onNavigate }) => {
  const { user, history = [], logout, updateProfileImage } = useAuth();
  const { t } = useLanguage();
  ...
  const compressImage = (base64Str, maxWidth = 150) => {
  ...
  const userHistory = history || user.history || [];
  const totalScans = userHistory.length;
  const detectedIssues = userHistory.filter(h => {
    const pestName = (h.pest || h.diseaseName || '').toLowerCase();
    return pestName !== 'healthy' && (h.confidence || 0) > 0.5;
  }).length;
  const healthyCrops = Math.max(0, totalScans - detectedIssues);
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-stone-200 overflow-hidden">
          ...
          <div className="px-8 pb-8 relative">
            <div className="flex flex-col md:flex-row items-start md:items-end -mt-12 mb-6 gap-6">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap items-center gap-4 mt-2 text-stone-600 text-sm">
                  <span className="flex items-center gap-1"><MapPin size={14} /> {user.farmLocation || 'India'}</span>
                  <span className="flex items-center gap-1"><Ruler size={14} /> {user.farmSize || 5} Acres</span>
                  <span className="flex items-center gap-1"><Calendar size={14} /> Joined {formatDate(user.createdAt || user.joinedDate || new Date().toISOString()).split(',')[0]}</span>
                </div>
              </div>
              ...
              <button 
                ...
                className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-stone-200 text-stone-700 bg-stone-50 hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-all text-sm font-bold shadow-sm"
              >
                <LogOut size={16} />
            ...
            <div className="flex items-center gap-2 border-t border-stone-100 pt-6">
              ...
              {(user.primaryCrops ? user.primaryCrops.split(',') : ['Rice', 'Wheat']).map((crop, i) => (
          ...
          {userHistory.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {userHistory.map((item, index) => {
                ...
                const cropName = item.crop || (item.fullAnalysis && item.fullAnalysis.crop) || 'Crop';
                const pestName = item.pest || item.diseaseName || (item.fullAnalysis && item.fullAnalysis.identification && item.fullAnalysis.identification.name) || 'Healthy';
                const severity = item.severity || 'low';
                const confidence = item.confidence || 0.95;
                const previewImg = item.imagePreview || item.imageUrl || item.image;
                ...
                return (
                  <div 
                    ...
                    onClick={() => {
                      if (item.fullAnalysis) {
                        setSelectedReport(item.fullAnalysis);
                      } else {
                        // Reconstruct full analysis object for viewing
                        setSelectedReport({
                          crop: cropName,
                          identification: {
                            name: pestName,
                          ...
                          weatherImpact: item.weatherImpact || {
                            riskLevel: 'Moderate',
                            summary: 'Normal field conditions',
                            recommendation: 'Monitor crop status'
                          }
                        });
                      }
                    }}
                  ...
                  >
                    <div className="h-48 overflow-hidden relative bg-gray-100">
                      {previewImg ? (
                        <img src={previewImg} alt={cropName} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
                      ) : (
                    ...
                    <div className="p-5">
                      <div className="flex justify-between items-start mb-2">
                        <h3 className="font-bold text-stone-900 text-lg group-hover:text-emerald-600 transition-colors">{pestName}</h3>
                      ...
                      <div className="flex items-center justify-between text-sm text-stone-500 mt-4">
                        <span className="flex items-center gap-1"><Sprout size={14} /> {cropName}</span>
        ...
        {selectedReport && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/80 backdrop-blur-sm animate-fade-in" onClick={() => setSelectedReport(null)}>
            ...
            >
              ...
              <div className="-mt-20">
                <ResultCard data={selectedReport} variant="modal" />
              </div>
            </div>
          </div>
        )}
